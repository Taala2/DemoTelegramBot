...
async def track_price() -> str:
    try:
        # Check cache first
        now = datetime.utcnow()
        if price_cache and all(now - timestamp < CACHE_DURATION for _, timestamp in price_cache.values()):
...
def format_prices(prices: Dict[str, float]) -> str:
    return "\n".join([f'Price of {sym}: {price:.2f} USDT' for sym, price in prices.items()])
